    ...
    public class ImportPictureCmd extends SimpleCommand 
    {
        ...
        private function fileCompleteHandler(e:Event):void 
        {
            ...
            loader.contentLoaderInfo.addEventListener(Event.COMPLETE, function(e:Event):void { 
                ...
                var bmpdate:BitmapData = Bitmap(loader.content).bitmapData;
                
                if (bmpdate.height > MAX_SIZE || bmpdate.width > MAX_SIZE)
                {
                    //TODO scale
                    var scale:Number = bmpdate.width < bmpdate.height?MAX_SIZE / bmpdate.height:MAX_SIZE/bmpdate.width;
                    
                    var matrix:Matrix = new Matrix();
                    matrix.scale(scale, scale);
                    
                    var bitmapCopy:BitmapData = new BitmapData(bmpdate.width*scale,bmpdate.height*scale,true,0);
                    bitmapCopy.draw(bmpdate, matrix,null,null,null,true);
                ...
                if (bmpdate)
                {
                    var img:Image = new Image(Texture.fromBitmapData(bmpdate));
